...
define("POLISNINJA_CRONVERSION","1");
...
function generate_masternodeslistfull_json_files($mysqli, $testnet = 0) {
    ...
    $data = array('status' => 'OK',
        'data' => array('masternodes' => $nodes,
            ...
            'api' => array('version' => 3,
                'compat' => 3,
                'bev' => 'mnfl='.POLISNINJA_BEV.'.'.POLISNINJA_CRONVERSION)
        ));
...
function generate_blocks24h_json_files($mysqli, $testnet = 0) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("polisninja_protocolesc_%d",$testnet);
    ...
    if ($result = $mysqli->query($sql)) {
        ...
        $data = array('status' => 'OK',
            ...
            'api' => array(
                'version' => 3,
                'compat' => 1,
                'bev' => 'bk24h='.POLISNINJA_BEV.".".POLISNINJA_CRONVERSION
            )
        ));
...
function generate_nodesstatus_json_files($mysqli, $testnet = 0) {
    ...
    $data = array('status' => 'OK',
        'data' => array('nodes' => $nodes,
            ...
            'api' => array('version' => 3,
                'compat' => 3,
                'bev' => 'mnst='.POLISNINJA_BEV.'.'.POLISNINJA_CRONVERSION)
        ));
...
function generate_blocksconsensus_json_files($mysqli, $testnet = 0) {
    ...
    $data = array('status' => 'OK',
        'data' => array('blocksconsensus' => $bhinfofinal,
            ...
            'api' => array('version' => 3,
                'compat' => 3,
                'bev' => 'bkcs='.POLISNINJA_BEV.'.'.POLISNINJA_CRONVERSION)
        ));
...
function generate_governancevotelimit_json_files($mysqli, $testnet = 0) {
    ...
    $data = array('status' => 'OK',
        ...
        'api' => array(
            'version' => 1,
            'compat' => 1,
            'bev' => 'gpvl=' . POLISNINJA_BEV . "." . POLISNINJA_CRONVERSION
        )
    ));
...
function generate_governanceproposals_json_files($mysqli, $testnet = 0) {
    ...
    echo "OK (".$currentblock["BlockId"]." / Estimated budget amount: ".$estimatedbudgetamount." POLIS)\n";
    ...
    $data = array('status' => 'OK',
        ...
        'api' => array(
            'version' => 2,
            'compat' => 1,
            'bev' => 'gp='.POLISNINJA_BEV.".".POLISNINJA_CRONVERSION
        )
        ));
...
function generate_governancetriggers_json_files($mysqli, $testnet = 0) {
    ...
    $data = array('status' => 'OK',
        ...
        'api' => array(
            'version' => 1,
            'compat' => 1,
            'bev' => 'gt='.POLISNINJA_BEV.".".POLISNINJA_CRONVERSION
        )
        ));
    ...
    $data = array("status" => 'OK',
        'data' => array('superblocks' => $superblocks,
            ...
            'api' => array(
                'version' => 1,
                'compat' => 1,
                'bev' => 'sb='.POLISNINJA_BEV.".".POLISNINJA_CRONVERSION
            )
        ));
...
xecho('POLIS Ninja Front-End JSON Generator cron v'.POLISNINJA_BEV.'.'.POLISNINJA_CRONVERSION."\n");
