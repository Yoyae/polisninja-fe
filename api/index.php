...
$app->get('/api/blocks', function() use ($app,&$mysqli) {
  ...
  if ($request->hasQuery('pubkeys')) {
    ...
    if (($mnpubkeys === false) || !is_array($mnpubkeys)) {
      ...
      foreach ($mnpubkeys as $mnpubkey) {
        if ( ( ($testnet == 1) && ! ((substr($mnpubkey,0,1) == 'y') ) )
          || ( ($testnet == 0) && ! (substr($mnpubkey,0,1) == 'P') )
          || ( strlen($mnpubkey) != 34 ) ) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("polisninja_blocks_%d_%d_%s_%d_%d_%d_%s",$testnet,$cachenodetail,$cacheinterval,count($mnpubkeys),$onlysuperblocks,count($budgetids),$cacheserial);
      ...
      if ($result = $mysqli->query($sql)) {
        ...
        $data = array('blocks' => $blocks,
            ...
            'api' => array(
                ...
                'bev' => 'bk='.POLISNINJA_BEV.".".$apiversion
            )
                                                                         );
        //Change the HTTP status
        $response->setStatusCode(200, "OK");
...
$app->get('/api/blocks/superblocks', function() use ($app,&$mysqli) {
    ...
    if (count($errmsg) > 0) {
        ...
        $cachefnam = CACHEFOLDER.sprintf("polisninja_blocks_superblockspayments_%d_%d_%s",$testnet,count($proposalshash),$cacheserial);
        $cachefnamupdate = $cachefnam.".update";
        $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+$cachetime)>=time()) || file_exists($cachefnamupdate)));
        if ($cachevalid) {
            $data = unserialize(file_get_contents($cachefnam));
            ...
            if ($result = $mysqli->query($sql)) {
                ...
                $data = array('superblocks' => $superblocks,
                    ...
                    'api' => array(
                        'version' => $apiversion,
                        'compat' => $apiversioncompat,
                        'bev' => 'sb='.POLISNINJA_BEV.".".$apiversion
                    )
                );
                //Change the HTTP status
                $response->setStatusCode(200, "OK");
...
$app->get('/api/budgets', function() use ($app,&$mysqli) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("polisninja_budgets_%d_%d_%d_%d_%s",$testnet,$onlyvalid,count($budgetids),count($budgethashes),$cacheserial);
    $cachefnamupdate = $cachefnam.".update";
      ...
      if ($result = $mysqli->query($sql)) {
        ...
        $data = array('budgets' => $budgets,
            ...
            'api' => array(
                'version' => $apiversion,
                'compat' => $apiversioncompat,
                'bev' => 'bu='.POLISNINJA_BEV.".".$apiversion
            )
                     );
...
$app->get('/api/budgetsexpected', function() use ($app,&$mysqli) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cachefnam = CACHEFOLDER . sprintf("polisninja_budgets_final_%d", $testnet);
    $cachefnamupdate = $cachefnam . ".update";
        ...
        if ($result = $mysqli->query($sql)) {
          ...
          $data = array(
              ...
              'api' => array(
                  'version' => $apiversion,
                  'compat' => $apiversioncompat,
                  'bev' => 'be=' . POLISNINJA_BEV . "." . $apiversion
              )
          );
...
$app->get('/api/budgets/votes', function() use ($app,&$mysqli) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("polisninja_budgets_votes_%d_%d_%s",$testnet,$onlyvalid,$cacheserial);
    $cachefnamupdate = $cachefnam.".update";
    $cachetime = filemtime($cachefnam);
    $cachevalid = (is_readable($cachefnam) && ((($cachetime+120)>=time()) || file_exists($cachefnamupdate)));
    if ($cachevalid) {
      ...
      if ($result = $mysqli->query($sql)) {
        ...
        $data = array('budgetsvotes' => $budgetsvotes,
            ...
            'api' => array(
                'version' => $apiversion,
                'compat' => $apiversioncompat,
                'bev' => 'bv='.POLISNINJA_BEV.".".$apiversion
            )
        );
...
$app->get('/api/budgetsprojection', function() use ($app,&$mysqli) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cacheserial = sha1(serialize($budgetids).serialize($budgethashes));
    $cachefnam = CACHEFOLDER.sprintf("polisninja_budgetsprojection_%d_%d_%d_%d_%s",$testnet,$onlyvalid,count($budgetids),count($budgethashes),$cacheserial);
    $cachefnamupdate = $cachefnam.".update";
    $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+120)>=time()) || file_exists($cachefnamupdate)));
    if ($cachevalid) {
      $data = unserialize(file_get_contents($cachefnam));
      ...
      if ($result = $mysqli->query($sql)) {
        ...
        $data = array('budgetsprojection' => $budgets,
            ...
            'api' => array(
                'version' => $apiversion,
                'compat' => $apiversioncompat,
                'bev' => 'bp='.POLISNINJA_BEV.".".$apiversion
            )
        );
...
$app->get('/api/governanceproposals', function() use ($app,&$mysqli) {
    ...
    if (count($errmsg) > 0) {
        ...
        $cachefnam = CACHEFOLDER.sprintf("polisninja_governanceproposals_%d_%d_%d_%d_%s",$testnet,$onlyvalid,count($proposalsnames),count($proposalshashes),$cacheserial);
        $cachefnamupdate = $cachefnam.".update";
        $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+120)>=time()) || file_exists($cachefnamupdate)));
        if ($cachevalid) {
            $data = unserialize(file_get_contents($cachefnam));
            ...
            if ($result = $mysqli->query($sql)) {
                ...
                $data = array('governanceproposals' => $proposals,
                    ...
                    'api' => array(
                        'version' => $apiversion,
                        'compat' => $apiversioncompat,
                        'bev' => 'gp='.POLISNINJA_BEV.".".$apiversion
                    )
                );
...
$app->get('/api/governanceproposals/votelimit', function() use ($app,&$mysqli) {
    ...
    $cachefnam = CACHEFOLDER.sprintf("polisninja_governanceproposals_votelimit_%d",$testnet);
    $cachefnamupdate = $cachefnam.".update";
    $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+120)>=time()) || file_exists($cachefnamupdate)));
    if ($cachevalid) {
        $data = unserialize(file_get_contents($cachefnam));
        ...
        $data = array('votelimit' => array(
            ...
            'api' => array(
                'version' => $apiversion,
                'compat' => $apiversioncompat,
                'bev' => 'gpvl=' . POLISNINJA_BEV . "." . $apiversion
            )
        );
...
$app->get('/api/governanceproposals/votes', function() use ($app,&$mysqli) {
    ...
    if (count($errmsg) > 0) {
        ...
        $cacheserial = sha1(serialize($budgetid));
        $cachefnam = CACHEFOLDER.sprintf("polisninja_governanceproposals_votes_%d_%s",$testnet,$cacheserial);
        $cachefnamupdate = $cachefnam.".update";
        $cachetime = filemtime($cachefnam);
        $cachevalid = (is_readable($cachefnam) && ((($cachetime+120)>=time()) || file_exists($cachefnamupdate)));
        if ($cachevalid) {
            ...
            if ($result = $mysqli->query($sql)) {
                ...
                $data = array('governanceproposalvotes' => $governanceproposalvotes,
                    ...
                    'api' => array(
                        'version' => $apiversion,
                        'compat' => $apiversioncompat,
                        'bev' => 'gpv='.POLISNINJA_BEV.".".$apiversion
                    )
                );
...
$app->get('/api/governancetriggers', function() use ($app,&$mysqli) {
    ...
    if (count($errmsg) > 0) {
        ...
        $cachefnam = CACHEFOLDER.sprintf("polisninja_governancetriggers_%d_%d_%d_%d",$testnet,$onlyvalid,$onlyfuture,$afterblockheight);
        $cachefnamupdate = $cachefnam.".update";
        $cachevalid = (is_readable($cachefnam) && (((filemtime($cachefnam)+120)>=time()) || file_exists($cachefnamupdate)));
        if ($cachevalid) {
            $data = unserialize(file_get_contents($cachefnam));
            ...
            if ($result = $mysqli->query($sql)) {
                ...
                $data = array('governancetriggers' => $triggers,
                    ...
                    'api' => array(
                        'version' => $apiversion,
                        'compat' => $apiversioncompat,
                        'bev' => 'gt='.POLISNINJA_BEV.".".$apiversion
                    )
                );
...
//   prev12=0|1 (optional, respond as pre v0.12 Polis Ninja API, obsolete)
...
$app->get('/api/masternodes', function() use ($app,&$mysqli) {
  ...
  if ($protocol == -1) {
    $cachefnam = CACHEFOLDER.sprintf("polisninja_maxprotocol_%d",$testnet);
...
$app->get('/api/tablevars', function() use ($app,&$mysqli) {
  ...
  if (count($errmsg) > 0) {
    ...
    $cachefnam = CACHEFOLDER."polisninja_tablevars";
    $cachefnamupdate = $cachefnam.".update";
      ...
      if ($result = $mysqli->query($sql)) {
        ...
        $data = array("tablevars" => $stats,
            ...
            'api' => array(
                'version' => $apiversion,
                'compat' => $apiversioncompat,
                'bev' => 'tv='.POLISNINJA_BEV.".".$apiversion
            )
        );
