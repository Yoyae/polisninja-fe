...
// Polis Ninja Front-End (polisninja-fe) - Blocks (v2)

var polisninjaversion = '2.5.1';
...
$(document).ready(function(){
  ...
  if (typeof polisninjator !== 'undefined') {
      $('a[name=polisninjatorurl]').attr("href", "http://"+polisninjator+"/blocks.html");
      $('span[name=polisninjatordisplay]').show();
  }

  if (typeof polisninjai2p !== 'undefined') {
      $('a[name=polisninjai2purl]').attr("href", "http://" + polisninjai2p + "/blocks.html");
   ...
   $('#blockstable').on('xhr.dt', function ( e, settings, json ) {
        // Show global stats
        $('#globalsupplyamount').text( addCommas( json.data.stats.global.SupplyAmount) +' '+polisninjacoin[polisninjatestnet] );
        $('#globalmnamount').text( addCommas( json.data.stats.global.MNPaymentsAmount) +' '+polisninjacoin[polisninjatestnet] );
   ...
   tableBlocks = $('#blockstable').dataTable( {
        ajax: { url: "/data/blocks24h-"+polisninjatestnet+".json",
                dataSrc: 'data.blocks' },
