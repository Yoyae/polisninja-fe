...
// Polis Ninja Front-End (polisninja-fe) - Budget Details

var polisninjaversion = '1.2.3';
var tableVotes = null;
...
function budgetdetailsRefresh(useHash){
  ...
  var query = '/api/budgets?testnet='+polisninjatestnet;
  if (useHash) {
    ...
    $('#budgetstatus').text(result);
       ...
       if (polisninjatxexplorer[polisninjatestnet].length > 0) {
           ...
           for ( var i=0, ien=polisninjatxexplorer[polisninjatestnet].length ; i<ien ; i++ ) {
               if (ix == 0) {
                   outtxt += '<a href="'+polisninjatxexplorer[polisninjatestnet][0][0].replace('%%a%%',data.data.budgets[0].FeeHash)+'">'+data.data.budgets[0].FeeHash+'</a>';
               }
               else {
                   outtxt += '<a href="'+polisninjatxexplorer[polisninjatestnet][i][0].replace('%%a%%',data.data.budgets[0].FeeHash)+'">['+ix+']</a>';
               }
               ix++;
           }
       }
       ...
       $('#budgetmonthlyamount').html( addCommas( data.data.budgets[0].MonthlyPayment.toFixed(3) )+' '+polisninjacoin[polisninjatestnet] + ' (<span id="budgetmonthlyamountusd">???</span> USD) (<span id="budgetmonthlyamounteur">???</span> EUR)');
       $('#budgettotalamount').html( addCommas( data.data.budgets[0].TotalPayment.toFixed(3) )+' '+polisninjacoin[polisninjatestnet] + ' (<span id="budgettotalamountusd">???</span> USD) (<span id="budgettotalamounteur">???</span> EUR)' );
       ...
       outtxt = "";
           if (polisninjaaddressexplorer[polisninjatestnet].length > 0) {
               ...
               for ( var i=0, ien=polisninjaaddressexplorer[polisninjatestnet].length ; i<ien ; i++ ) {
                   if (ix == 0) {
                       outtxt += '<a href="'+polisninjaaddressexplorer[polisninjatestnet][0][0].replace('%%a%%',data.data.budgets[0].PaymentAddress)+'">'+data.data.budgets[0].PaymentAddress+'</a>';
                   }
                   else {
                       outtxt += '<a href="'+polisninjaaddressexplorer[polisninjatestnet][i][0].replace('%%a%%',data.data.budgets[0].PaymentAddress)+'">['+ix+']</a>';
                   }
                   ix++;
               }
           }
       ...
       if (tableVotes !== null) {
           ...
           tableVotes = $('#votestable').dataTable({
               ajax: {
                   url: '/api/budgets/votes?testnet=' + polisninjatestnet + '&budgetid=' + encodeURIComponent(budgetid) + '&onlyvalid=1',
       ...
       if (tableSuperBlocks !== null) {
           ...
           tableSuperBlocks = $('#superblockstable').dataTable({
               ajax: {
                   url: '/api/blocks?testnet=' + polisninjatestnet + '&budgetids=["' + encodeURIComponent(budgetid) + '"]&onlysuperblocks=1',
                   dataSrc: 'data.blocks'
               },
               lengthMenu: [[50, 100, 250, 500, -1], [50, 100, 250, 500, "All"]],
               pageLength: 50,
...
function refreshBudgetProjection(useHash) {
    ...
    $.getJSON(query, function (data) {
        ...
        if ((data.hasOwnProperty("data")) && (data.data.hasOwnProperty("budgetsprojection")) && (Array.isArray(data.data.budgetsprojection)) && (data.data.budgetsprojection.length == 1) &&
            ...
            $('#budgetstatus').html("Valid, Established and Alloted (" + addCommas(data.data.budgetsprojection[0].Alloted.toFixed(3)) + " " + polisninjacoin[polisninjatestnet] + ")");
        }
        else {
...
$(document).ready(function(){
  ...
  if ((budgetid == "") && (budgethash == "")) {
    ...
    if ((budgetid != "") && (budgethash == "")) {
      if (!polisbudgetregexp.test(budgetid)) {
          budgetid = 'Invalid';
      ...
      if (!polisoutputregexp.test(budgethash)) {
          budgethash = 'Invalid';
