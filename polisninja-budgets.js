...
// Polis Ninja Front-End (polisninja-fe) - Budgets

var polisninjaversion = '1.5.1';
...
$(document).ready(function(){
    ...
    $('#budgetsdetailtable').on('xhr.dt', function ( e, settings, json ) {
        ...
        $('#globalestablishedbudgetamount').text(addCommas(totalamount)+' '+polisninjacoin[polisninjatestnet]);
...
//        $('#globalnextsuperblockunallocated').text(addCommas(Math.round((json.data.stats.nextsuperblock.estimatedbudgetamount-totalamount)*100)/100)+' '+polisninjacoin[polisninjatestnet]);
    ...
    tableBudgets = $('#budgetsdetailtable').dataTable( {
        ajax: { url: "/api/budgets?testnet="+polisninjatestnet,
        ...
        columns: [
            ...
            { data: null, render: function ( data, type, row ) {
                var outtxt = data.ID;
                if (type != 'sort') {
                    outtxt = '<a href="'+polisninjabudgetdetail[polisninjatestnet].replace('%%b%%',data.ID)+'">'+data.ID+'</a>';
                }
                return outtxt;
            } },
            { data: null, render: function ( data, type, row ) {
                ...
                if (type == 'sort') {
                    ...
                    if (polisninjatxexplorer[polisninjatestnet].length > 0) {
                        ...
                        for ( var i=0, ien=polisninjatxexplorer[polisninjatestnet].length ; i<ien ; i++ ) {
                            if (ix == 0) {
                                outtxt += '<a href="'+polisninjatxexplorer[polisninjatestnet][0][0].replace('%%a%%',data.FeeHash)+'">'+data.FeeHash.substring(0,7)+'</a>';
                            }
                            else {
                                outtxt += '<a href="'+polisninjatxexplorer[polisninjatestnet][i][0].replace('%%a%%',data.FeeHash)+'">['+ix+']</a>';
                            }
                            ix++;
                        }
                    }
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == 'sort') {
                    ...
                    return addCommas(data.TotalPayment+' '+polisninjacoin[polisninjatestnet]);
                }
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == 'sort') {
                    ...
                    return addCommas(data.MonthlyPayment+' '+polisninjacoin[polisninjatestnet]);
                }
                ...
                if (type == 'sort') {
                    ...
                    if (polisninjaaddressexplorer[polisninjatestnet].length > 0) {
                        ...
                        for ( var i=0, ien=polisninjaaddressexplorer[polisninjatestnet].length ; i<ien ; i++ ) {
                            if (ix == 0) {
                                outtxt += '<a href="'+polisninjaaddressexplorer[polisninjatestnet][0][0].replace('%%a%%',data.PaymentAddress)+'">'+data.PaymentAddress+'</a>';
                            }
                            else {
                                outtxt += '<a href="'+polisninjaaddressexplorer[polisninjatestnet][i][0].replace('%%a%%',data.PaymentAddress)+'">['+ix+']</a>';
                            }
                            ix++;
                        }
                    }
    ...
    $('#budgetsprojectiondetailtable').on('xhr.dt', function ( e, settings, json ) {
        ...
        $('#globalallotedbudgetamount').text(addCommas(totalamount)+' '+polisninjacoin[polisninjatestnet]);
        $('#globalnextsuperblockunallocated').text(addCommas(Math.round((json.data.stats.nextsuperblock.estimatedbudgetamount-totalamount)*100)/100)+' '+polisninjacoin[polisninjatestnet]);
    ...
    tableBudgetsProjection = $('#budgetsprojectiondetailtable').dataTable( {
        ajax: { url: "/api/budgetsprojection?testnet="+polisninjatestnet,
        ...
        columns: [
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == 'sort') {
                    ...
                    return addCommas(data.Alloted+' '+polisninjacoin[polisninjatestnet]);
                }
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == 'sort') {
                    ...
                    return addCommas(data.TotalBudgetAlloted+' '+polisninjacoin[polisninjatestnet]);
                }
    ...
    tableSuperBlocks = $('#superblockstable').dataTable( {
        ajax: { url: "/api/blocks?testnet="+polisninjatestnet+"&onlysuperblocks=1",
            dataSrc: 'data.blocks' },
        paging: false,
        order: [[ 0, "desc" ]],
        columns: [
...
//                return deltaTimeStampHR(currenttimestamp(),data.BlockTime);
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == "sort") {
                    ...
                    return '<a href="' + polisninjabudgetdetail[polisninjatestnet].replace('%%b%%',encodeURIComponent(data.SuperBlockBudgetName)) + '">' + data.SuperBlockBudgetName + '</a>';
                }
              }
            },
            { data: null, render: function ( data, type, row ) {
                if (type == "sort") {
                    ...
                    return addCommas(data.BlockMNValue.toFixed(3))+" "+polisninjacoin[polisninjatestnet];
                }
            }
            }
        ],
    ...
    tableSuperBlocksExpected = $('#superblocksexpectedtable').dataTable( {
        ajax: { url: "/api/budgetsexpected?testnet="+polisninjatestnet,
        ...
        columns: [
            ...
            { data: null, render: function ( data, type, row ) {
                if (type == "sort") {
                    ...
                    return '<a href="' + polisninjabudgetdetail[polisninjatestnet].replace('%%b%%',encodeURIComponent(data.BlockProposal)) + '">' + data.BlockProposal + '</a>';
                }
            }
            },
            { data: null, render: function ( data, type, row ) {
                if (type == "sort") {
                    ...
                    return addCommas(data.MonthlyPayment.toFixed(3))+" "+polisninjacoin[polisninjatestnet];
                }
            }
            }
        ],
