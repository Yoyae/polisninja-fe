...
// Polis Ninja Front-End (polisninja-fe) - Masternode Detail

var polisninjaversion = '3.2.1';
var tablePayments = null;
var tableExStatus = null;
var dataProtocolDesc = [];
var maxProtocol = -1;
var polismainkeyregexp = /^[P][a-zA-Z0-9]{33}$/;
var polistestkeyregexp = /^[y][a-zA-Z0-9]{33}$/;
var polisoutputregexp = /^[a-z0-9]{64}-[0-9]+$/;
var mnpubkey = '';
var mnvin = '';
...
function mndetailsRefresh(useVin){
  ...
  var query = '/api/masternodes?balance=1&portcheck=1&lastpaid=1&exstatus=1&testnet='+polisninjatestnet;
  if (useVin) {
    query += '&vins=["'+mnvin+'"]';
  }
  else {
  ...
  $.getJSON( query, function( data ) {
   ...
   if ((!data.hasOwnProperty("data")) || (data.data.length < 1)) {
    ...
    $('#mnbalance').text ( addCommas( num.toFixed(3) )+' '+polisninjacoin[polisninjatestnet]).removeClass("danger").removeClass("success").addClass(cls);
    ...
    if (data.data[0].LastPaidFromBlocks !== false) {
      ...
      if (polisninjaqueryexplorer[polisninjatestnet].length > 0) {
        outtxt += '<a href="'+polisninjaqueryexplorer[polisninjatestnet][0][0].replace('%%q%%',data.data[0].LastPaidFromBlocks.MNLastPaidBlock)+'">'+data.data[0].LastPaidFromBlocks.MNLastPaidBlock+'</a>';
      }
      else {
    ...
    if ((data.data[0].hasOwnProperty("Portcheck")) && (data.data[0].Portcheck != false)) {
        if ((data.data[0].Portcheck.SubVer.length > 11) && (data.data[0].Portcheck.SubVer.substring(0, 12) == '/Polis Core:') && (data.data[0].Portcheck.SubVer.substring(data.data[0].Portcheck.SubVer.length - 1) == '/')) {
            versioninfo = data.data[0].Portcheck.SubVer.substring(12, data.data[0].Portcheck.SubVer.indexOf('/', 12));
        }
        else if ((data.data[0].Portcheck.SubVer.length > 11) && (data.data[0].Portcheck.SubVer.substring(0, 12) == '/polis Core:') && (data.data[0].Portcheck.SubVer.substring(data.data[0].Portcheck.SubVer.length - 1) == '/')) {
            versioninfo = data.data[0].Portcheck.SubVer.substring(12, data.data[0].Portcheck.SubVer.indexOf('/', 12));
        }
    }
    else {
   ...
   $('#mninfosLR').text( n + ' ' + time );
      ...
      tablePayments = $('#paymentstable').dataTable( {
        responsive: true,
        searching: false,
        ajax: { url: '/api/blocks?testnet='+polisninjatestnet+'&pubkeys=["'+data.data[0].MasternodePubkey+'"]&interval=P1M',
                dataSrc: 'data.blocks',
                cache: true },
        paging: false,
        order: [[ 0, "desc" ]],
        columns: [
            ...
            { data: null, render: function ( data, type, row ) {
               if ((type != "sort") && (data.BlockMNPayeeExpected == "")) {
                 ...
                 return '<a href="'+polisninjamasternodemonitoring[polisninjatestnet].replace('%%p%%',data.BlockMNPayeeExpected)+'">'+data.BlockMNPayeeExpected+'</a>';;
               }
            } },
            { data: null, render: function ( data, type, row ) {
               if ((type != "sort") && (data.BlockMNPayee == "")) {
                 ...
                 return '<a href="'+polisninjamasternodemonitoring[polisninjatestnet].replace('%%p%%',data.BlockMNPayee)+'">'+data.BlockMNPayee+'</a>';;
               }
            } }
        ],
...
$(document).ready(function(){
  ...
  if ((mnpubkey == "") && (mnvin == "")) {
    ...
    if ((mnpubkey != "") && (mnvin == "")) {
      if (((polisninjatestnet == 0) && (!polismainkeyregexp.test(mnpubkey)))
        || ((polisninjatestnet == 1) && (!polistestkeyregexp.test(mnpubkey)))) {
        mnpubkey = 'Invalid';
        $('#mnpubkey').text(mnpubkey);
      }
      else {
      ...
      if (!polisoutputregexp.test(mnvin)) {
        mnvin = 'Invalid';
        $('#mnoutput').text( mnvin );
      }
      else {
  ...
  $('#paymentstable').on('xhr.dt', function ( e, settings, json ) {
        ...
        $('#mntotalpaid').text ( addCommas( num.toFixed(3) )+' '+polisninjacoin[polisninjatestnet]+' ('+numpaid+' times / '+missed+' missed / '+hijacked+' hijacked)');
